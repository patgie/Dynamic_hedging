...
class Net_LV(nn.Module):
    ...
    def __init__(self, device,n_strikes, n_networks):
        ...
        # initialise price diffusion neural network (same neural network for each maturity)
        self.S_vol =  Net_timegrid(dim=2, nOut=1, n_layers=3, vNetWidth=150, n_networks=1, activation_output="softplus")
    ...
    def forward(self, S0, realised_prices,past_hedges, rate, z, MC_samples, ind_T,n_maturities, maturities,maturity_exotic, strikes, timegrid,gradient_count): 
        ...
        for i in range(1, ind_T+1):
            ...
            price_diffusion = self.S_vol.forward_idx(0, torch.cat([t,Slog_old],1))
            price_diffusion_const = price_diffusion.detach()

            if cal_day<i and args.hedge_exotic:
                price_diffusion_exotic = self.S_vol.forward_idx(0, torch.cat([t_shift,Slog_old_exotic],1))
    ...
    for epoch in range(n_epochs):
        ...
        if epoch==0:
            ...
            if args.hedge_exotic==False:
                filename = "NSDE_model_initial_prices_LV_seed_{}_cal_day_{}.pth.tar".format(seed,cal_day)
        ...
        if iv_mean_error < best_IV_mean_error and args.hedge_exotic==False:
            ...
            filename = "NSDE_val_LV_seed_{}_cal_day_{}.pth.tar".format(seed,cal_day)
        ...
        if  epoch==(n_epochs-1):
            ...
            if args.hedge_exotic==False:
                checkpoint_str= "NSDE_val_LV_seed_{}_cal_day_{}.pth.tar".format(seed,cal_day)
            if args.lookback_exotic==True and args.hedge_exotic==True:
            ...
            if args.hedge_exotic==False:
                filename = "NSDE_test_LV_seed_{}_cal_day_{}.pth.tar".format(seed,cal_day)
            if args.lookback_exotic==True and args.hedge_exotic==True:
...
if __name__ == '__main__':
    ...
    model = Net_LV( device=device,n_strikes=n_strikes, n_networks=1)
    model.to(device)
   ...
   # for name, param in model.named_parameters():
    ...
    if cal_day==0:
       ...
       # checkpoint_str= "NSDE_test_LV_seed_{}_cal_day_{}.pth.tar".format(seed,cal_day) 
      #  checkpoint=torch.load(checkpoint_str)
      #  model.load_state_dict(checkpoint['state_dict'])
        model = model.to(device)  
